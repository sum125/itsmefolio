...
class _ProjectsDetailPageState extends State<ProjectsDetailPage> {
  ...
  static const Map<String, Map<String, dynamic>> _data = {
    ...
    'lgsuite': {
      ...
      'pivot':
          '4개 클러스터 모두가 후보는 아니었습니다. 국내 당일치기 여행객은 숙소 체류 시간이 짧아 개입할 맥락 자체가 부족했고, 여행 기록·공유형은 숙소를 콘텐츠 배경으로만 쓸 뿐 회복이 핵심 가치로 작동하지 않았습니다. 장거리 해외여행 일정 조율형도 고려했지만, 이들의 페인포인트는 동선·교통편 같은 이동 문제에 가까웠습니다.\n\n반면 숙소 품질·위치 비교형은 실제 데이터에서 **관광에서 숙소 결정이 가장 중요하다**고 말하고 있었고, **여행의 피로는 이동에서부터 누적된다 — 숙소 도착이 곧 회복의 시작이어야 한다**는 인사이트가 나왔습니다. 이에 따라, 객실 안의 가전 경험을 설계하는 LG SUITE를 제안했습니다.',
      'decision':
          '**여행을 완주하고 싶지만 어쩔 수 없이 숙소로 돌아오게 되는 순간, 그 또한 여행의 큰 즐거움이 되었으면 좋겠다**는 이 페르소나의 바람을, **일상처럼 편안하고 회복되는 숙박 경험**이라는 서비스 가치로 바꿔 제안했습니다. 가전을 전면에 내세우지 않고, 체크인 전부터 실내 환경을 선제적으로 세팅해 여행객이 도착하자마자 다음 여행 일정을 위한 회복을 시작할 수 있게 설계했습니다.',
      'retrospective':
